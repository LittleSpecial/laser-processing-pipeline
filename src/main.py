...
def main(input_dir, output_dir, filename_prefix=None, delay_time=None, manual_center_x=None, manual_center_y=None):
    ...
    if p1_aligned is None or p2_aligned is None: return
    save_image_safely(output_dir / f"{base_name}P1_aligned_full.png", p1_aligned)
    save_image_safely(output_dir / f"{base_name}P2_aligned_full.png", p2_aligned)
    print("图像配准完成。")
    ...
    if manual_center_x is not None and manual_center_y is not None:
        ...
        # 在手动模式下，我们没有精确的ROI掩码，因此创建一个覆盖整个图像的“伪掩码”
        # crop_and_center_roi 函数会从中裁剪出需要的部分
        ...
        if roi_mask_original_size is None:
            print("错误: 自动ROI分割失败。")
            return None, None # 返回None以通知batch_process失败
...
if __name__ == '__main__':
    ...
    main(Path(args.input_dir), Path(args.output_dir), args.filename_prefix, args.delay, args.manual_center_x, args.manual_center_y)